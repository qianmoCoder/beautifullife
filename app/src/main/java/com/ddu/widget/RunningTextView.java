...
public class RunningTextView extends androidx.appcompat.widget.AppCompatTextView {

    public int content;// 最后显示的数字
    private int frames = 25;// 总共跳跃的帧数,默认25跳
    private int nowNumber = 0;// 显示的时间
        ...
        handler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                RunningTextView.this.setText(formater.format(nowNumber)
                        .toString());// 更新显示的数字
                nowNumber += Integer.parseInt(msg.obj.toString());// 跳跃arg1那么多的数字间隔
//				Log.v("nowNumber增加之后的值", nowNumber + "");
                //
                if (nowNumber < content) {
                    Message msg2 = handler.obtainMessage();
                    msg2.obj = msg.obj;
                    handler.sendMessageDelayed(msg2, 70);// 继续发送通知改变UI
                } else {
     ...
     * @param moneyNumber
     */
    public void playNumber(int moneyNumber) {
        if (moneyNumber == 0) {
        ...
        thread_pool.execute(new Runnable() {
            ...
            public void run() {
                ...
                int temp = content / frames;
                msg.obj = temp <= 1 ? 1 : temp;// 如果每帧的间隔比1小，就设置为1
