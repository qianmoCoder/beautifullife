...
// #MARK - Constructors
    ...
    public MagnificentChart(@NonNull Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.MagnificentChart, 0, 0);
        try {
            boolean isAnimated = typedArray.getBoolean(R.styleable.MagnificentChart_m_animation, false);
            boolean isRound = typedArray.getBoolean(R.styleable.MagnificentChart_m_round, false);
            boolean showShadow = typedArray.getBoolean(R.styleable.MagnificentChart_m_shadow, true);
            boolean showTitle = typedArray.getBoolean(R.styleable.MagnificentChart_m_showTitle, false);
            int shadowColor = typedArray.getColor(R.styleable.MagnificentChart_m_shadowColor, Color.parseColor("#F2F2F2"));
            int backgroundColor = typedArray.getColor(R.styleable.MagnificentChart_m_background, Color.parseColor("#FFFFFF"));
...
// #MARK - Drawing Methods
    ...
    private void regularDraw(@NonNull Canvas canvas) {
    ...
    private void animatedDraw(@NonNull Canvas canvas) {
        Paint insideShadowPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        insideShadowPaint.setColor(shadowBackgroundColor);
        Paint insideChartPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        insideChartPaint.setColor(chartBackgroundColor);
    ...
    private void drawInsideCircle(@NonNull Canvas canvas, @NonNull Paint insideShadowPaint, @NonNull Paint insideChartPaint) {
        if (!isRound) {
            if (isShadowShowing) {
                canvas.drawCircle(width / 2, height / 2, width / 4 - 10, insideShadowPaint);
            }
    ...
    private void drawMainCircle(@NonNull Canvas canvas, @NonNull Paint insideShadowPaint, @NonNull Paint insideChartPaint, @NonNull RectF mainRectangle) {
        if (isShadowShowing) {
    ...
    private void drawItems(@NonNull Canvas canvas, @NonNull RectF mainRectangle) {
        float startAngle = 0f;
        float anglesSum = 0f;
