...
public class Banner<T> extends LinearLayout {
    ...
    private ICorePageChangeListener pageChangeListener;
    private ViewPager.OnPageChangeListener onPageChangeListener;
    private ICorePageAdapter pageAdapter;
    ...
    public Banner(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ICoreBanner);
        canLoop = a.getBoolean(R.styleable.ICoreBanner_canLoop, true);
        a.recycle();
        init(context);
    }
    ...
    static class AdSwitchTask implements Runnable {
        ...
        AdSwitchTask(Banner ICoreBanner) {
            this.reference = new WeakReference<>(ICoreBanner);
        }

        @Override
        public void run() {
            Banner ICoreBanner = reference.get();

            if (ICoreBanner != null) {
                if (ICoreBanner.viewPager != null && ICoreBanner.turning) {
                    int page = ICoreBanner.viewPager.getCurrentItem() + 1;
                    ICoreBanner.viewPager.setCurrentItem(page);
                    ICoreBanner.postDelayed(ICoreBanner.adSwitchTask, ICoreBanner.autoTurningTime);
                }
            }
        }
    }

    public Banner setPages(ViewHolderCreator holderCreator, List<T> datas) {
        setAdapter(holderCreator, datas, false);
        return this;
    }

    public Banner setCustomerPages(ViewHolderCreator holderCreator, List<T> datas) {
        setAdapter(holderCreator, datas, true);
        return this;
    }

    private void setAdapter(ViewHolderCreator holderCreator, List<T> datas, boolean isCustomer) {
        try {
            int size = datas.size();
            this.mDatas = datas;
    ...
    public Banner setPageIndicator(int[] page_indicatorId) {
        ...
        pageChangeListener = new ICorePageChangeListener(mPointViews,
                page_indicatorId);
...
//        //如果是正在翻页的话先停掉
     ...
     * @return
     */
    public Banner setOnICorePageChangeListener(ViewPager.OnPageChangeListener onPageChangeListener) {
    ...
    public int getRealCount() {
        return viewPager.getAdapter().getRealCount();
    }
